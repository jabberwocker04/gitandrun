...
### ● 데이터베이스
<img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white"> <img src="https://img.shields.io/badge/Amazon RDS-527FFF?style=flat&logo=amazonrds&logoColor=white">
...
- 주문 조회
  - **공통** : 주문 상세 조회
  - **고객** : 내 주문 조회
    - 정렬 및 필터링 : 가게 이름별 / 가격 높은 순 / 가격 낮은 순 / 최신순 / 오래된 순 / 주문 상태별 / 주문 타입별
  - **사장** : 내 가게 주문 조회
    - 조회 범위 : 가게 전체 주문 조회 / 가게 단일 주문 조히
    - 정렬 및 필터링 : 고객 이름별 / 각격 높은 순 / 가격 낮은 순 / 최신순 / 오래된 순 / 주문 상태별 / 주문 타입별
  - **관리자 / 매니저** : 전체 주문 조회
     -  정렬 및 필터링 : 가게 이름별 / 고객 이름별 / 각격 높은 순 / 가격 낮은 순 / 최신순 / 오래된 순 / 주문 상태별 / 주문 타입별 / 삭제 여부
...
- 결제 조회
  - **공통** : 결제 상세 조회
  - **고객** : 내 결제 조회
    - 정렬 및 필터링 : 가게 이름별 / 가격 높은 순 / 가격 낮은 순 / 최신순 / 오래된 순 / 결제 상태별
  - **관리자 / 매니저** : 전체 결제 조회
     -  정렬 및 필터링 : 가게 이름별 / 고객 이름별 / 각격 높은 순 / 가격 낮은 순 / 최신순 / 오래된 순 / 결제 상태별 / 삭제 여부
